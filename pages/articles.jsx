...
export async function getStaticProps() {
	...
	const data = files
		.map((filename) => {
			const isFile = fs.statSync(path.join('markdown/articles', filename)).isFile();
			...
			const dir = fs.readdirSync(path.join('markdown/articles', filename), 'utf-8')
			const files = dir.filter(nestedFile => {
				return fs.statSync(
					path.join('markdown/articles', filename, nestedFile)
				)
				.isFile();
			});
			return {
